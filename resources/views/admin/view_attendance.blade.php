		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-sm-12">
		            ...
		            <div class="alert alert-block alert-danger" id="alert_no_school_classes">
		                <button type="button" class="close" data-dismiss="alert">
		                    <i class="ace-icon fa fa-times"></i>
		                </button>
		                <span id="no_school_classes_message"><b>{{session('myuser_schools')[2]['school_name']}} Has No Classes !...</b></span>
		            </div>
		            @endif

		            <div class="alert alert-block alert-danger" id="alert_no_school_classes2" style="display: none;">
		                <button type="button" class="close" data-dismiss="alert">
		                    <i class="ace-icon fa fa-times"></i>
		                </button>
		                <span id="no_school_classes_message2"></span>
		            </div>
                ...
                <div class="col-xs-12">
                    ...
                    <div class="col-sm-8">
                        ...
                        <div class="profile-user-info profile-user-info-striped">
                            ...
                            <div class="profile-info-row" id="div_school" style="display: none;">
                                <div class="profile-info-name">School Name</div>
                                <div class="profile-info-value">
                                	<label id="get_school_name"><b>{{session('myuser_schools')[2]['school_name']}}</b></label>
                                	<input type="hidden" id="school_weekend" value="{{$weekend_day_name}}">
                                	<input type="hidden" name="school_id" id="school_id" value="{{session('myuser_schools')[2]['school_id']}}">
			                    </div>
			                </div>
                            <div class="profile-info-row" style="display:none;" id="div_class">
                                ...
                                <div class="profile-info-value">
                                	<select class="form-control" id="class_school_id" style="color:#333333;font-weight:bold;font-size:13px;">
                                    	...
                                    	<option value="{{$class->id}}" class_id="{{$class->sms_class_id}}"> {{$class->class}} </option>
                                    	@endforeach
                                 	</select> 
			                    </div>
			                </div>
                            ...
                            <div class="profile-info-row" id="div_show_attendance" style="display:none;">
		                        ...
		                        <div class="profile-info-value">
		                            <span class="editable editable-click">
		                                <button id="show_attendance" class="btn btn-xl btn-info" style="float:right;">Show Attendance</button>
		                            </span>
		                        </div>
						    </div>   
	                    </div>   
			...
			<div class="row" id="show_attendance_information" style="display: none">
			</div>
		</div>
	</div>
</div>
...
/*Attendance Table & Button*/		
		...
		$(document).on('click','#show_attendance',function(){
			...
			var school_name = $("#get_school_name").text();
                ...
                success: function(data)
                	{
                        ...
                        {
                            $('#alert_no_school_classes2').show();
                            $("#no_school_classes_message2").html("<b>"+data.daily_attendance_date+" Has Been Assigned To Holiday ("+data.holiday_title+")<br/>{ "+data.holiday_description+" }!...</b>");
                            $("#show_attendance_information").hide();
                        }
                        ...
                        {
                            $('#alert_no_school_classes2').show();
                            $("#no_school_classes_message2").html("<b> This Is "+data.weekend_day_name+" That Is School Weekend Off Day!...</b>");
                            $("#show_attendance_information").hide();
                        }
                        ...
                        {
                            $('#alert_no_school_classes2').show();
                            $("#no_school_classes_message2").html("<b> Attendance For "+data.daily_attendance_date+" Has Not Been Taken !...</b>");
                            $("#show_attendance_information").hide();
                        }
                        else
                        {
            ...
            else if(report_type_id == 2)
            {
				...
				/*Get attendance recored of monthly individual*/	
				...
				if(get_month != ""){
 					...
 					$('#error_message_select_month').hide();
	 				 	...
	 				 	$.ajax({
					        url:"monthly_individual_function_admin_side",
					        type:"POST",
					        data:{_token: '{{csrf_token()}}',class_school_id:class_school_id,month_year:get_month},
					        success:function(ResponseText){
					        
					        	//this condition for check attendacne takened in this month year
					        	if(ResponseText.message == 'fail'){
					        		
					        	
					        		$("#alert_no_school_classes2").show();
					        		
					        		$("#no_school_classes_message2").html("<b> Attendance For "+month_name+" Has Not Been Taken!...</b>");
					        		$("#no_school_classes_message2").show();
					        		$('#show_attendance_information').html("");
					        		...
					        		$('#show_attendance_information').show();
					        		$("#alert_no_school_classes2").hide();
					        		$("#no_school_classes_message2").hide();
					        	}
            ...
            else if(report_type_id == 4)
            {
            	...
            	$.ajax({
		    		...
		    		success:function(data){
		    			...
		    			{
		    				$('#alert_no_school_classes2').show();
		    				$('#no_school_classes_message2').html("<b>Attendance For "+data.year+" Has Not Been Taken!...</b>");
			            	$('#show_attendance_information').html('');
			            	$('#show_attendance_information').hide();
		    			}
		    			else
		    			{
		    				$("#alert_no_school_classes2").hide();
		    				$("#no_school_classes_message2").html('');
		    				$('#show_attendance_information').html(data);
			            	$('#show_attendance_information').show();
		    			}
            ...
            else if(report_type_id == 5)
            {
				...
				{		
              	 	$.ajax({
					        url:"generate_individual_attendance_report_by_class_school_id_date_range_school_admin",
					        type:"POST",
					        ...
					        success:function(data){
								if(data.message == "fail")
								{
									...
									$('#alert_no_school_classes2').show();
									$('#no_school_classes_message2').html("<b>Attendance From "+date_from+" To "+date_to+" Has Not Been Taken!...</b>");
									$('#no_school_classes_message2').show();									
									$("#show_attendance_information").hide();
									$("#daily_attendance_date_picker").hide();
									$("#alert_no_school_is_selected").hide();
            ...
            else if(report_type_id == 7)
            {
				...
				else
				{
              	 	...
              	 	$.ajax({
					        url:"/admin/generate_average_attendance_report_by_school_id_date_range",
					        type:"POST",
					        data:{_token: '{{csrf_token()}}',
                            school_id:school_id,
                            school_name:school_name,
					        ...
					        success:function(data)
					        {
					        	...
					        	if(data.message=='fail')
								{
					        		...
					        		$("#show_attendance_information").html('');
					        		$("#alert_no_school_classes2").show();
					        		$("#no_school_classes_message2").html("<b>Attendance Has Not Been Taken From: "+data.date_from+" To: "+data.date_to+"</b>");
					        	}
					        	else
					        	{
					        		$('#show_attendance_information').show();
