                    ...
                    <div class="col-sm-8">
                        <div class="profile-user-info profile-user-info-striped">
                            ...
                            <div class="profile-info-row" id="div_school" style="display: none;">
                                ...
                                <div class="profile-info-value">
                                	<label id="get_school_name"><b>{{session('myuser_schools')[3]['school_name']}}</b></label>
                                	<input type="hidden" id="school_weekend" value="{{$weekend_day_name}}">
                                	<input type="hidden" name="school_id" id="school_id" value="{{session('myuser_schools')[3]['school_id']}}">
			                    </div>
			                </div>
                            <div class="profile-info-row" id="div_class" style="display:none;">
                                ...
                                <div class="profile-info-value">
                                     <select class="form-control"  id="class_school_id"  style="color:#333333;font-weight:bold;font-size:13px;">
                                            ...
                                            <option class_id="{{$teacher_class->class_id}}" value="{{$teacher_class->sms_class_school_id}}">{{$teacher_class->class}}</option>
											@endforeach
                                    </select> 
                                </div>
                            </div>
		...
		$(document).on('click','#show_attendance',function(){
                ...
                success: function(data)
                	{
                        ...
                        {
                            $('#alert_no_school_classes2').show();
                            $("#no_school_classes_message2").html("<b>"+data.daily_attendance_date+" Has Been Assigned To Holiday ("+data.holiday_title+"</b> ["+data.holiday_description+"] <b>)</b>");
                            $("#show_attendance_information").hide();
                        }
                        ...
                        {
                            $('#alert_no_school_classes2').show();
                            $("#no_school_classes_message2").html("<b>"+data.weekend_day_name+" That Is School Weekend Off Day!...</b>");
                            $("#show_attendance_information").hide();
                        }
            ...
            else if(report_type_id == 2)
            {
				...
				if(get_month != ""){
 					...
 					$('#error_message_select_month').hide();
	 				 	...
	 				 	$.ajax({
					        url:"monthly_individual_function_teacher_side",
					        type:"POST",
            ...
            else if(report_type_id == 5)
            {
				...
				else
				{
              	 	...
              	 	$.ajax({
					        url:"generate_individual_attendance_report_by_class_school_id_date_range_teacher",
					        type:"POST",
            ...
            else if(report_type_id == 7)
            {
                ...
                if (Date.parse(date_from) > Date.parse(date_to)) {
                    ...
                    $.ajax({
                        url: "/teacher/generate_average_attendance_report_by_school_id_date_range",
                        type: "POST",
                        data: {
                            _token: '{{csrf_token()}}',
                            school_id: school_id,
